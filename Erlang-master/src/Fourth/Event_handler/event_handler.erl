...
-author("makst").
-export([init/1, start/2, stop/1, add_handler/3, delete_handler/2, get_data/2, send_event/2, swap_handlers/3]).
...
handle_msg({swap_handlers, OldHandler, NewHandler}, LoopData) ->
  case lists:keysearch(OldHandler, 1, LoopData) of
    ...
    {value, {OldHandler, Data}} ->
      OldData = OldHandler:terminate(Data),
      NewLoopData = [{NewHandler, NewHandler:init(OldData)} |
        lists:keydelete(OldHandler, 1, LoopData)],
      {{data, OldData}, NewLoopData}
  end;
