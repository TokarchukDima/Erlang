-module(observer).
-author("makst").
-export([init/1, start_link/2, stop/1, start_child/4, stop_child/2]).

-define(RestartCount, 5).
-define(Duration, 300).
...
restart_child(Pid, ChildList, Reason) ->
  ...
  case {Type, Reason} of
    ...
    _Other ->
      ...
      if
        length(NewRestartTime) > ?RestartCount ->
...
restart_update(RestartTime) ->
  ...
  [Now | lists:filter(fun(E) -> (Now-E) < ?Duration end, RestartTime)].
